...
// --- the tool round: an exchange the reviewer is not part of -------------------
//
// Ken, 2026-09-15, pasting a card from review rec7752d7cefe: "This should not be
// showing up in my chat rail." The card showed the page check asking for a
// data-lahe-id and, under it, the agent's bare "handled". Both were right. It
// was still the wrong thing to put in front of a person: the id is plumbing
// between the tool and the agent, and the reviewer cannot act on any of it.

/** The stamp reopen, as the page check makes it. */
function stampReopen(store, rail, done) {
  const item = store.write(REVIEW, readyItem({ note: "shorten this heading" }));
  rail.upsertCard(item);
  done.applyReplies([foldEvent(item)]);
  return done.reopen(item.id, {
    note: record.PAGE_CHECK_STAMP_NOTE,
    notice: null,
...
test("the page check's stamp reopen is marked a tool round, on the record", () => {
  const { store, rail, done } = setup();
  const reopened = stampReopen(store, rail, done);

  assert.equal(reopened.state, record.STATE.READY, "the agent really does have work");
  assert.equal(record.toolRoundOf(reopened), record.TOOL_ROUND.PAGE_CHECK_STAMP);
  assert.equal(reopened.region.check_reopen.tool, record.TOOL_ROUND.PAGE_CHECK_STAMP);
  assert.match(reopened.note, /data-lahe-id stamp did not reach the source/, "the agent still gets asked");
});

test("a revert reopen is the reviewer's business, so it is not a tool round", () => {
  const { store, rail, done } = setup();
  const item = store.write(REVIEW, readyItem());
  rail.upsertCard(item);
  done.applyReplies([foldEvent(item)]);
  const reopened = done.reopen(item.id, { note: record.PAGE_CHECK_NOTE, pageCheck: true });

  assert.equal(record.toolRoundOf(reopened), null, "their own words went missing: that is theirs to know");
  assert.equal(rail.getCard(item.id).pane, overlay.TAB.ACTIVE);
  assert.equal(rail.getCard(item.id).state, record.STATE.READY);
});

test("a tool round leaves the card exactly where the reviewer left it", () => {
  const { store, rail, done } = setup();
  const reopened = stampReopen(store, rail, done);

  const card = rail.getCard(reopened.id);
  assert.equal(card.pane, overlay.TAB.DONE, "still in Done");
  assert.equal(card.state, record.STATE.HANDLED, "still reading handled");
  assert.equal(card.notice, null, "and saying nothing new");
  assert.equal(record.displayState(reopened), record.STATE.HANDLED);
  assert.equal(overlay.paneForItem(reopened), overlay.TAB.DONE, "the one pane rule agrees");
});

test("the agent's answer to a tool round is never news, whatever it says", () => {
  const { store, rail, done } = setup();
  const reopened = stampReopen(store, rail, done);

  // Even flagged, with words on it, which is the loudest an ordinary reply gets.
  const loud = foldEvent(reopened, {
    payload: {
      accepted: true,
      state: record.STATE.HANDLED,
      file: "replies-claude.jsonl",
      reply: Object.assign({}, flaggedReply(), { files: [] })
    }
  });
  const applied = done.applyReplies([loud]);
  assert.equal(applied[0].kind, "folded");
  assert.equal(applied[0].toast, false, "no toast");

  const answered = store.readItem(REVIEW, reopened.id);
  assert.deepEqual(tabDone.unseenReplyIds([answered], {}), [], "no unseen mark, so no badge and no 1 new");
  const card = rail.getCard(reopened.id);
  assert.equal(card.state, record.STATE.HANDLED);
  assert.equal(card.pane, overlay.TAB.DONE);
  assert.equal(card.agentMessage, null, "and nothing drawn for the reply itself");
  assert.equal(card.notice, null);
});

test("a refusal is the one thing a tool round says out loud, and it says it quietly", () => {
  const { store, rail, done } = setup();
  const reopened = stampReopen(store, rail, done);
    ...
    foldEvent(reopened, {
      payload: {
        ...
        state: record.STATE.NOT_HANDLED,
        file: "replies-claude.jsonl",
        reply: { status: "not_handled", agent: "claude", reason: "the template is generated by a plugin" }
      }
    })
  ]);

  const card = rail.getCard(reopened.id);
  assert.match(card.notice, /^The agent could not carry the id into the source: /);
  assert.match(card.notice, /generated by a plugin/, "with the agent's own reason, because a person may need it");
  assert.equal(card.agentMessage, null, "still no agent block, and still no round");
  const answered = store.readItem(REVIEW, reopened.id);
  assert.deepEqual(tabDone.unseenReplyIds([answered], {}), [], "and still no badge: it is a notice, not an alarm");
});

test("the archived tool round travels marked, so history draws nothing for it either", () => {
  const { store, rail, done } = setup();
  const reopened = stampReopen(store, rail, done);
  done.applyReplies([foldEvent(reopened)]);

  // The reviewer follows up later, which archives the tool's round into history.
  const followed = done.followUp(reopened.id, "one more thing about this heading");
  const rounds = record.threadOf(followed);
  assert.equal(rounds.length, 2, "the reviewer's own round, then the tool's");
  assert.equal(record.isToolRound(rounds[0]), false, "the first was theirs");
  assert.equal(rounds[1].tool, record.TOOL_ROUND.PAGE_CHECK_STAMP);
  assert.equal(record.isToolRound(rounds[1]), true);
  assert.equal(
    rounds.filter((round) => !record.isToolRound(round)).length,
    1,
    "which is the one round the rail draws"
  );
});

...
test("cannot tell is never turned into nobody", () => {
  // A machine with no lsof on it answers null. Saying "no agent connected" there
...
test("the quiet indicator answers one question: is the chain intact", () => {
  ...
  assert.equal(rail.statusLine().text, "Stored · agent connected");
  ...
  assert.equal(rail.statusLine().text, "Stored · no agent connected");
...
test("the line stays quiet for the first two minutes, then says how long", () => {
  ...
  // No agent connected is loud from the moment it is known, because the
...
test("a review with no agent session is told the truth, and never accused of nobody", () => {
  ...
  // from, so "no agent connected" would be an alarm about nobody. The wait is

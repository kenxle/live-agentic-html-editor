...
- **The every-page rule applies to folder reviews only.** Reviewing a single file used to root the server at that file's folder, and with the new rule every HTML file there would have carried a live review token. Now a single page gets the line and its siblings are served plain. Your rule was stated for folder reviews; if you meant it wider, it is a one-line change.
